...
GURUH_USLUBI = (
    "Sen 'Mening yordamchim' botsаn. Sen o'zbek yigitlari guruhida yashaysan. "
...
def _call_gemini(contents: str, system_instruction: str | None = None) -> str:
    """Gemini ni chaqiradi. Google Search yoqilgan — real vaqt ma'lumotlari uchun."""
...
def safe_reply(message, text: str):
    """Xavfsiz reply — xabar o'chirilgan bo'lsa indamaymiz."""
    try:
        return bot.reply_to(message, text)
    except Exception as e:
        logger.warning("safe_reply xato (xabar o'chirilganmi?): %s", e)
        return None


def safe_edit(chat_id, message_id, text: str):
    """Xavfsiz edit — xabar yo'q bo'lsa indamaymiz."""
    try:
    ...
    try:
        ...
        if wait_msg:
            safe_edit(
                message.chat.id,
                wait_msg.message_id,
                "Hozir AI serveri band. Bir necha soniyadan so'ng qayta urinib ko'ring.",
            )


def gemini_chat(message, query: str):
    """Guruh uslubida do'stona javob berish."""
    query = query.strip()
    if not query:
        return
...
def handle_all_messages(message):
    try:
        ...
        if NAME_PATTERN.search(text):
            safe_reply(
                message,
                "Abdulaziz hozir band oshna \U0001f60a Gaping bo'lsa menga ayt!",
            )
            return
...
def _keep_alive_loop():
    """Har 4 daqiqada tashqi URLga ping yuborib Replit ni uyg'otib turadi."""
...
def main():
    ...
    # Health server — har doim PORT da ishlaydi (deploy uchun ham)
    ...
    # Keep-alive — Replit ni uyg'otib turadi
    ...
    # Webhook ni o'chirib polling rejimida ishla
    time.sleep(3)
    try:
        bot.remove_webhook()
    except Exception as e:
